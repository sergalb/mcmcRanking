...
```{r message=FALSE}
install.packages("/Users/Sergey.Balakhin/Desktop/diploma/gatom_0.0.0.9000.tar.gz", repos = NULL, type="source")
library(BioNet)
library(DLBCL)
library(genefilter)
library(impute)
...
g <- makeMetabolicGraph(network=networkEx,
                        topology = "atoms",
                        org.gatom.anno=org.Mm.eg.gatom.annoEx,                          gene.de=gene.de.rawEx,
                        met.db=met.kegg.dbEx,
                        met.de=met.de.rawEx)
g <- scoreGraph(g, k.gene=25, k.met=NULL)
V(g)$likelihood <- 1
...
y <- sample_llh(graph = g, 10, niter = 1e5, exp_lh = 1/2^depth)
